    ...
    public class USBE
    {
        ...
        public string Platform_CalculateSingature_String(int PhDeviceHandle, int DataType, byte[] strpcData, int size, ref string strSingBody)
        {
            ...
            try
            {
                ...
                string pp = "";
                foreach (byte item in pucCounter)
                {
                    pp += item.ToString("X2") + " ";
                }
                ...
                string pp1 = "";
                foreach (byte item in signdata)
                {
                    pp1 += item.ToString("X2") + " ";
                }
                // MessageBox.Show(pp1);
                string ww= pp1.ToUpper();
        ...
        public bool Platform_CalculateSingature_Byte(int PhDeviceHandle, int DataType, byte[] strpcData, int size, ref byte[] strSingBody)
        {
            string CalculateSignaturestr = string.Empty;
            try
            {
                byte[] pucCounter = new byte[100];
                int pucSignCerSn = 100;
                 int nResule = Platform_CalculateSignature(ref PhDeviceHandle, 1, strpcData, size, pucCounter, ref pucSignCerSn);//目前用的是1
                string pp = "";
                foreach (byte item in pucCounter)
                {
                    pp += item.ToString("X2") + " ";
                ...
                strSingBody = Convert.FromBase64String(CalculateSignaturestr);
